...
return $r.strftime(fromTime,fmt);}else{return days+" days ago"}}}})})(jQuery);(function($){$.fn.spamjax=function(callback,settings){var settings=settings||{}
var options={}
...
window.onresize=function(){GitHub.positionTriangle()}
window.onload=function(){GitHub.positionTriangle()}
...
$.extend(GitHub,{positionTriangle:function(){if($('#triangle').length==0)
$('body').append('<div id="triangle" style="position: absolute; display: none;"> </div>')
var d=$('#repo_menu .active')[0]
if(d){var tarray=$('#triangle').show()
var t=tarray[0];var offset=$(d).offset()
var height=$(d).outerHeight()
t.style.top=''+(offset.top+height)+'px';t.style.left=''+offset.left+'px';if($('#repo_menu .site').length>0)
var ref=$('#repo_menu .site')[0].offsetLeft;if($('#repo_sub_menu .site a').length>0)
$('#repo_sub_menu .site a')[0].style.marginLeft=(((d.offsetLeft-ref)/13.34)-1)+'em';}},scrollToHilightedLine:function(){var lines,target=window.location.hash
if(lines=target.match(/^#?(?:L|-)(\d+)$/g)){lines=$.map(lines,function(line){return parseInt(line.replace(/\D/g,''))})
$('#LID'+lines[0]).scrollTo(1)}}})
$(function(){GitHub.positionTriangle()
GitHub.scrollToHilightedLine()
$('.jshide').hide()
$('.toggle_link').click(function(){$($(this).attr('href')).toggle()
return false})
$('#beta :text').focus(function(){if($(this).val()=='your email')$(this).val('')})
if($.fn.ajaxForm){$('#beta form').ajaxForm(function(){$('#beta form').hide()
$('#beta .thanks').show()})}
$('.hide_alert').click(function(){$('#site_alert').slideUp()
$.cookie('hide_alert','t',{expires:7})
return false})
if($.cookie('hide_alert'))$('#site_alert').hide()
$('#login_field').focus()
...
$.user_autocomplete=function(){if(!$.fn.autocomplete)return
...
$.user_autocomplete()});$.ajaxSetup({'beforeSend':function(xhr){xhr.setRequestHeader("Accept","text/javascript")}})
...
$('.bubble.empty').hover(GitHub.Commit.dumpEmptyClass,GitHub.Commit.addEmptyClass)
$('.bubble').hover(GitHub.Commit.highlightLine,GitHub.Commit.unhighlightLine)
...
$('.bubble').click(function(){$(this).data('clicked',true)
GitHub.currentBubble=$(this)
var url=window.location.pathname.replace('commit','comments')
url+='/'+$.trim($(this).parents('.file').find('.meta').text())
var pos=parseInt($(this).attr('class').match(/cp-(\d+)/)[1])
url+='?position='+pos
if(pos){var firstLine=parseInt($(this).parents('tbody').find('.line_numbers:first > a:first').text())
url+='&line='+(firstLine+pos-1)}
$.gitbox(url)})
$('.add_comment').livequery('click',function(){var text=$.trim($('#commit_comment_form textarea').val())
if(text==''){$('#commit_comments .inner').scrollTo('#commit_comment_form')}else{$('.add_comment').attr('disabled',true).spin()
...
$('.add_comment').attr('disabled',false).stopSpin()
...
$.hotkeys({c:function(){GitHub.Commits.link('c')},p:function(){GitHub.Commits.link('p')},t:function(){GitHub.Commits.link('t')},j:function(){GitHub.Commits.next()},k:function(){GitHub.Commits.prev()}})});$(function(){$('#invite_to_beta').focus(function(){$(this).val('')})
$('#beta_invite_form').spamjax(function(text,status){var inside='#invite_'+status
var email=$('#invite_to_beta').val()
$.facebox($(inside).text()+email)
if(status=='success'){var left=$('#invites_remaining').text()
$('#invites_remaining').text(parseInt(left)-1)
$('#invite_to_beta').val('')
$('#invited_emails').append('<li>'+email+'</li>')}},{facebox:true})
$('.repo_filter').click(function(){$('#repo_listing li').hide()
$('#repo_filter a').removeClass('filter_selected')
$(this).addClass('filter_selected')
$('#repo_listing li.'+$(this).attr('rel')).show()
return false})
$('.betas h1').css('cursor','pointer').click(function(){$('#invited_emails').toggle()})});GitHub.editableGenerator=function(options){return function(_,self){var defaults={id:'field',tooltip:'Click to edit!',indicator:'Saving...',data:function(data){return $(self).attr('data')||data},style:"display: inline",onblur:function(){var reset=function(){$(self).trigger('reset').next().show()}
$.data(self,'timeout',reset.delay(20))},callback:function(){(function(){if($(self).attr('data'))$(self).attr('data',$(self).text())
$(self).trigger('truncate').next().show()}).delay(20)}}
return $(this).editable($(this).attr('rel'),$.extend({},defaults,options))}}
$(function(){$('.edit_link').click(function(){$(this).prev().trigger('click')
return false})});$(function(){$('.repo span.edit').each(GitHub.editableGenerator({width:'350px'}))
...
$('.hidden_message').show()
return false})});$(function(){$('#add_key').click(function(){$('#public_keys textarea:last').after($('#public_keys textarea:first').clone().html(''))
return false})
$('#remove_key').click(function(){if($('#public_keys textarea').length==1){$('#public_keys textarea').val('')}else{$('#public_keys textarea:last').val('').remove()}
return false})
$('#remove_key').mouseover(function(){$('#public_keys textarea:last').css('background','#ffc')})
$('#remove_key').mouseout(function(){$('#public_keys textarea:last').css('background','#fff')})});$(function(){$('#signup_form').keydown(function(e){if($(this).find(':submit').attr('disabled')&&e.keyCode==13)return false})
$('#signup-terms').click(function(){$('#signup-button').attr('disabled',!this.checked)})
$('#signup-button').click(function(){$(this).attr('disabled',true)
$(this).val('Creating your GitHub account...')
$('#signup_form').submit()})});$(function(){$('.relatize').relatizeDate()});$(function(){$('a.follow').click(function(){$.post(this.href,'')
...
$('#update_cc_form').submit()})});$(function(){var swap_search_and_links=function(){$('#header .userbox .site_links').toggle()
$('#header .userbox form').toggle()
$(this).is(':visible')
setTimeout(function(){$('#header :text').focus()},50)
return false}
$('#header .userbox .search_repos_link').livequery('click',swap_search_and_links)
$('#header .userbox .cancel_search_link').livequery('click',swap_search_and_links)
$.hotkey('s',function(){$('.search_repos_link').click()})});